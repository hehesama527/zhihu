...
function buildTopicAgentSingleSelectionPromptSuffix() {
  return [
    ...
    "5. 不要再把纯量化工作流作为选题方向。题目如果主要讨论量化策略上线、深度优化、参数调优、研究 pipeline 或团队研发效率，而不是币圈交易者的真实交易痛点，应设为 SKIP 或低优先级。",
    "6. 泛交易心态题可以选，例如外汇交易者为什么亏损、如何避免成为韭菜这类题；但它们更适合作为纯经验分享，不要默认加入 CryptoPathX。",
    "7. 对泛交易心态题，除非题目明确问工具、回测、监控、策略验证或复盘系统，否则 should_include_soft_promo=false，soft_promo_mode=none，topic_fingerprint.promo_entry=none。",
    "8. 软广需要有节奏，不要每一篇都带。整体按约 10 条里 7 条自然带产品、3 条纯分享/纯经验/纯观点来控制。",
    "9. 即使题目能勉强接到产品，只要它更适合做人设信任、交易心态、踩坑经验、币圈常识或观点判断，就应优先留白：should_include_soft_promo=false。",
    "10. 不要让纯量化题天然优先；如果币圈/心态/形态/风控/AI 交易联动题更符合账号 Soul 和读者痛点，可以给更高 priority 和 fit_score。",
    "11. 选题结束后，必须单独判断这个选题是否适合自然加入软广。",
    "12. 如果适合，把 should_include_soft_promo 设为 true，soft_promo_mode 设为 light 或 natural，并在 soft_promo_directive.writer_instruction 里明确告诉 Writer：这篇需要自然加入 CryptoPathX。",
    "13. 如果不适合，把 should_include_soft_promo 设为 false，soft_promo_mode 设为 none，topic_fingerprint.promo_entry 写 none，并在 soft_promo_directive.writer_instruction 里明确告诉 Writer：这篇不强制加入软广，不要硬带 CryptoPathX。",
    "14. 只有当 CryptoPathX 的真实能力能解决题目里的具体一步时，才允许 should_include_soft_promo=true；不要因为业务目标需要推广就默认每篇都带。",
    "15. 必须输出 writing_plan，由 Topic Agent 决定正文长度、是否需要案例、是否需要算账、是否适合列表/短标题、哪些重点需要加粗。",
    "16. length_mode 选择规则：简单知识问答用 short；普通方法题用 standard；交易经历、弯路复盘、新手入门、小本金、策略方法论、软文承接空间大的题用 long。",
    "17. 字数规则：target_words_min 是 Writer 必须达到的硬下限；target_words_max 只是软参考，可以超过，不能为了压字数牺牲案例、算账和信息密度。",
    "18. 案例规则：只有题目适合故事化时 should_use_cases=true；没有真实输入证据时 case_style 用 typical_composite 或 contrast_cases，可以要求 Writer 写接近真实的复合案例，但不要要求伪造真实朋友经历。",
    "19. 数据规则：案例里的胜率、回撤、盈亏比、仓位、手续费、滑点等数字要贴近真实市场常识、保守且自洽，不要要求精确历史统计。",
    "20. 算账规则：涉及本金、成本、收益预期、回撤、仓位、手续费、策略有效性时 should_include_calculation=true。",
    "21. 加粗规则：standard/long 文章默认 should_use_bold=true，bold_targets 应指定 2-5 类重点，如核心结论、风险边界、算账结论、操作原则、产品边界。",
    "22. suggested_sections 是结构提示，不是要求 Writer 原样使用的标题；避免反复输出“先说结论/最后补一句”这类固定模板。",
