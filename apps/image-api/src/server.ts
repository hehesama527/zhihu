...
import { XWorkspaceRepository } from "@zhihu-mvp/x-core";
import {
  bindJobImageSchema,
...
app.setErrorHandler((error, _request, reply) => {
  ...
  reply.status(statusCode).send({
    error: {
      code: statusCode,
      message: error instanceof Error ? error.message : "Image API error."
    }
  });
});
...
app.get("/image-assets/:id/thumbnail", async (request, reply) => {
  ...
  if (!asset?.thumbnailPath) {
    throw createHttpError(404, "缩略图不存在。");
  }
...
app.get("/image-assets/:id/original", async (request, reply) => {
  ...
  const asset = await imageAssetService.getAsset(params.id);
  if (!asset) {
    throw createHttpError(404, "图片资产不存在。");
  }
...
const address = await app.listen({
  host: "127.0.0.1",
  port: config.imageApiPort
});

app.log.info({ address }, "image-api listening");
