  ...
  return (
    <div className="stack">
      <section className="page-header">
        <div>
          <h2>排期中心</h2>
          <p className="muted">固定工作日发文，每个账号在 08:00 到 20:00 之间随机分布 3 到 4 个时段，周末休息。</p>
        </div>
      </section>
          ...
          {slots.length ? (
            slots.map((slot) => (
              <tr key={slot.id}>
                ...
                <td>
                  {slot.publishJobId ? <Link href={`/zhihu/jobs/${slot.publishJobId}`}>{slot.title ?? `任务 #${slot.publishJobId}`}</Link> : "待分配"}
                </td>
              </tr>
            ))
          ) : (
